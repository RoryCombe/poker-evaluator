...
export class PokerEvaluator {
  private handRanks: Buffer;
  private threeCardConverter: ThreeCardConverter;

  constructor() {
    fs.readFile(
      path.join(__dirname, '../data/HandRanks.dat'),
      this.assignHandRanks,
    );
  ...
  private assignHandRanks(error: NodeJS.ErrnoException, handRanks: Buffer) {
    if (error) return; // TODO better error handling
  ...
  private evalCard(card: number): number {
    return this.handRanks.readUInt32LE(card * 4);
  }
}
